...
export class BigmapComponent implements OnInit {
    ...
    public confirmedCaseOption: any = {
        title: {
            text: '医院确诊量',
            textStyle: {
                color: '#73D0CC',
        ...
        yAxis: [
            {name: '确诊量',  axisLine: {lineStyle: {color: '#44C4EE'}}, type: 'value', splitLine: {show: false}},
            {name: '变化率',  axisLine: {lineStyle: {color: '#44C4EE', formatter: '{value}%'}}, type: 'value', splitLine: {show: false}}
        ],
        series: [
            {
                name: '确诊量',
    ...
    private findTopTenData(data: any, cityNames: string[], values: number[]) {
        let count = 0;
        for (let i = data.length - 1 ; i >= 0 && count++ < 10 ; i-- ) {
            cityNames.push(data[i].name);
            values.push(data[i].value[2]);
        }
    ...
    ngOnInit() {
        ...
        this.findTopTenData(this.chartData, this.topTenCityName, this.topTenCityValue);
    ...
    private convertData() {
        ...
        for (let i = 0; i < this.data.length; i++) {
            const geoCoord = this.geoCoordMap[this.data[i].name];
            if (geoCoord) {
                this.data[i].value = this.randomData();
                res.push({
    ...
    private displayProvinceMap(provinceName: string) {
        ...
        console.log('provinceChartEle status： ' + this.provinceChartEle)
